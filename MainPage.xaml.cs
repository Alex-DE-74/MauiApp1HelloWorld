...
public partial class MainPage : ContentPage
{
	...
	protected override async void OnAppearing()
	{
		...
		Dispatcher.Dispatch(async () =>
		{
			...
			// status test
			var xs = HyperOsOpenAi.HatHintergrundFensterRecht();
			await DisplayAlertAsync("Status", xs.ToString(), "OK");
...
private async void OnTestExercisePlanClicked(object sender, EventArgs e)
{
	...
	public void SetzeWecker(int sekundenBisAlarm)
	{
		...
		// NEU: Für den BootReceiver persistent abspeichern, damit die Zeit den Reboot überlebt!
		Microsoft.Maui.Storage.Preferences.Default.Set("GeplanteWeckerZeitMs", triggerAtMs);
	...
	private BerechtigungsManager _berechtigungsManager = new BerechtigungsManager();
	...
	public async Task SetzeWeckerV2(int sekundenBisAlarm)
	{
		...
		*/
		...
		// Der ausgelagerte Manager prüft die Kette. Wenn ein Menü öffnet, bricht er ab und wartet aufs Resume.
		// Bei der Rückkehr ruft er diese Methode über das Lambda rekursiv wieder auf.
		bool alleRechteDa = await _berechtigungsManager.PruefeAlleBerechtigungenAsync();
		
		if (!alleRechteDa)
			return; // Stoppt den Ablauf hier, solange Rechte fehlen
	...
	public async Task SetzeWeckerV2_working_alt(int sekundenBisAlarm)
	{
	/*
	var contextBo = Android.App.Application.Context;
	var intentBo = new Android.Content.Intent(
	...
	*/
	...
	// await HyperOs.PruefeUndOeffneAutostartAsync();//  
	await HyperOsOpenAi.PruefeUndOeffneEchtenHyperOsAutostartAsync();
